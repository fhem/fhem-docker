...
FHEM_REVISION_CURRENT="$( cd ./src/fhem; svn log -v ./tags | grep "A /tags/FHEM_" | sort | tail -n 1 | cut -d / -f 5 | cut -d : -f 2 | cut -d ")" -f 1 )"
...
for REVISION in ${FHEM_REVISION_CURRENT} ${FHEM_REVISION_LATEST}; do
  ...
  FHEM_REVISION=$( [[ "${REVISION}" == "$FHEM_REVISION_LATEST" && "$FHEM_REVISION_LATEST" != "$FHEM_REVISION_CURRENT" ]] && echo -n "$FHEM_REVISION_LATEST" || echo -n "" )
  VARIANT_FHEM="${FHEM_VERSION}$( [ -n "${FHEM_REVISION}" ] && echo -n "-s${FHEM_REVISION}" || echo -n "" )"
  VARIANT_IMAGE="${IMAGE_VERSION}$( [ -n "${IMAGE_BRANCH}" ] && echo -n "-${IMAGE_BRANCH}" || echo -n "" )"
  VARIANT="${VARIANT_FHEM}_${VARIANT_IMAGE}"

  # only build latest FHEM in dev tree
  [[ -n "${TRAVIS_BRANCH}" && "${TRAVIS_BRANCH}" == "dev" && "${REVISION}" != "$FHEM_REVISION_LATEST" ]] && continue
  ...
  if [ -n "${TRAVIS_BRANCH}" ]; then
    if [ "${TRAVIS_BRANCH}" == "master" ] || [ "${TRAVIS_BRANCH}" == "${TRAVIS_TAG}" ]; then
      if [ ${REVISION} == $FHEM_REVISION_LATEST ]; then
        TAG="beta"
      else
        TAG="latest"
      fi
