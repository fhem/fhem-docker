...
mk-sbuild --target=armel stretch
mk-sbuild --target=armhf stretch
mk-sbuild --target=arm64 stretch

curl -fsSL https://github.com/multiarch/qemu-user-static/releases/download/v3.1.0-2/x86_64_qemu-arm-static.tar.gz | tar zx -C ~/
curl -fsSL https://github.com/multiarch/qemu-user-static/releases/download/v3.1.0-2/x86_64_qemu-aarch64-static.tar.gz | tar zx -C ~/
...
sudo cp -f qemu-aarch64-static /var/lib/schroot/chroots/stretch-amd64-arm64/usr/bin

sbuild --chroot stretch-amd64-armhf --arch armhf -j8
